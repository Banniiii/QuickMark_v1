...
@Component({
  ...
  styleUrls: ['./s-signup.page.scss'],
  standalone: false,
})
export class SSignupPage implements OnInit {
  password: string = '';
  ...
  onFileSelected(event: any) {
    ...
    async registerUser() {
      ...
      try {
        ...
        const { data: dbData, error: dbError } = await this.supabaseService.client.from('students').insert([{
          id: authData.user?.id, 
